...
class GoogleSheets:
    ...
    def download_file_from_google_drive(spreadsheet_link, miss_worksheets=None, invert_miss_worksheets=False):
        ...
        print(f'{Ilvl(1)}End downloading.')
            ...
            if len(miss_worksheets) > 0:
                if invert_miss_worksheets:
                    if worksheets_names[i] not in miss_worksheets:
                        print(f'{Ilvl(2)}Worksheet: "{worksheets_names[i]}" {CCol.txt_yel("SKIPPED")}')
                        continue
                else:
                    if worksheets_names[i] in miss_worksheets:
                        print(f'{Ilvl(2)}Worksheet: "{worksheets_names[i]}" {CCol.txt_yel("SKIPPED")}')
                        continue
            print(f'{Ilvl(2)}Reading worksheet: "{worksheets_names[i]}".', end=' ')
            start = time.time()
            tables.append(spreadsheet.get(worksheets_names[i]).to_dict('records'))
            end = time.time() - start
            print(f'{Ilvl(1)}{CCol.txt_grn("DONE")}: ', round(end, 3))

        os.remove(file_name)
        print(f'{Ilvl(2)}Removed downloaded file.')
    ...
    def pars(spreadsheet_link, miss_worksheets=None, invert_miss_worksheets=False):
        ...
        try:
                ...
                if len(miss_worksheets) > 0:
                    if invert_miss_worksheets:
                        ...
                        if worksheets_names[i] in miss_worksheets:
                            print(f'{Ilvl(2)}Worksheet: "{worksheets_names[i]}" {CCol.txt_yel("SKIPPED")}')
                            continue
                print(f'{Ilvl(2)}Parsing worksheet: "{worksheets_names[i]}".', end=' ')
                start = time.time()
            ...
            print(f'{Ilvl(1)}{CCol.txt_yel(txt)}')
