...
# from api.old_base_api_requests import BaseApiRequests as ApiReq
...
def increase_data_values(i, data_values, options_increasing_data_values):
    ...
    for key, value in options_increasing_data_values.items():
        if value is not None:
            if i % value['step'] == 0:
                if type(result[key]) in (int, float):
...
class Start:
    ...
    def start_create_file(self):
        file_size = 1000000  # 1_000_000 23_000_000
        for k, v in {'_2gb': file_size}.items():
            # data_v = {
            #     'Plant': 1,
            #     'SKU': 10000,
            #     'SKU Name': 'SKU Name_1',
            ...
            options_increasing = {
                ...
                'sku': {'value': 1, 'step': 1},
                'wh': {'value': 1, 'step': 1},
                'chain': {'copy_value': 'sku', 'step': 1},
                'rfa_id': {'value': 1, 'step': 100},
                'ordered': {'value': 0.01, 'step': 1},
                'delivered': {'value': 0.01, 'step': 1}
            }
